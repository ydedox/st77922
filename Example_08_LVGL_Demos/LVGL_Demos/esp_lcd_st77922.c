...
#define ESP_LCD_ST77922_VER_PATCH (3)
...
static esp_err_t touch_st77922_read_data(esp_lcd_touch_handle_t tp);
static bool touch_st77922_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num);
static esp_err_t touch_st77922_del(esp_lcd_touch_handle_t tp);
static esp_err_t touch_st77922_reset(esp_lcd_touch_handle_t tp);

static esp_err_t i2c_read_bytes(esp_lcd_touch_handle_t tp, int reg, uint8_t *data, uint8_t len);
static esp_err_t touch_st77922_read_fw_info(esp_lcd_touch_handle_t tp);
...
static esp_err_t panel_st77922_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    tx_color(st77922, io, LCD_CMD_RAMWR, color_data, len);//2C
...
esp_err_t esp_lcd_touch_new_i2c_st77922(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    esp_err_t ret = ESP_OK;
    esp_lcd_touch_handle_t st77922 = (esp_lcd_touch_handle_t)calloc(1, sizeof(esp_lcd_touch_t));
    ESP_GOTO_ON_FALSE(st77922, ESP_ERR_NO_MEM, err, TAG, "Touch handle malloc failed");

    /* Communication interface */
    st77922->io = io;
    /* Only supported callbacks are set */
    st77922->read_data = touch_st77922_read_data;
    st77922->get_xy = touch_st77922_get_xy;
    st77922->del = touch_st77922_del;
    /* Mutex */
    st77922->data.lock.owner = portMUX_FREE_VAL;
    /* Save config */
    memcpy(&st77922->config, config, sizeof(esp_lcd_touch_config_t));
    ...
    if (config->int_gpio_num != GPIO_NUM_NC) {
        ...
        if (config->interrupt_callback) {
            esp_lcd_touch_register_interrupt_callback(st77922, config->interrupt_callback);
        }
    }
    ...
    /* Reset controller */
    ESP_GOTO_ON_ERROR(touch_st77922_reset(st77922), err, TAG, "Reset failed");
    ESP_GOTO_ON_ERROR(touch_st77922_read_fw_info(st77922), err, TAG, "Read version failed");

    ESP_LOGI(TAG, "Touch panel create success, version: %d.%d.%d", ESP_LCD_ST77922_VER_MAJOR, ESP_LCD_ST77922_VER_MINOR,
             ESP_LCD_ST77922_VER_PATCH);
...
static esp_err_t touch_st77922_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_read_bytes(tp, TOUCH_INFO, (uint8_t *)&adv_info, 1), TAG, "Read advanced info failed");
